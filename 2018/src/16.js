const fs = require('fs')
const input = String(fs.readFileSync( __dirname + '/../inputs/16.txt'))
    .trim()
...
const inputOperationsRaw = input.split('\n')
...
const inputOps = range(0, inputOperations.length, 4).reduce(
  (accl, i) => {
    return accl.concat({
      before: inputOperationsRaw[i].split('[')[1].substr(0, 10).split(', ').map(c => parseInt(c)),
    op: {
    ...
    after: inputOperationsRaw[i+2].split('[')[1].substr(0, 10).split(', ').map(c => parseInt(c))
    })
  }, [])
...
const testOps = opNames.reduce((tests, op) => ({
  ...tests,
...
function posibilities() {
  ...
  inputOps.forEach(({ after, before, op}) => {
    ...
    if (!possibleMappings[op.op]) {
      ...
      possibleMappings[op.op] = intersection(
        possibleMappings[op.op], new Set(matching)
      )
    }
  })
...
function findMapping () {
  ...
  while (Object.keys(foundMapping).length !== Object.keys(ops).length) {
    const options = posibilities()
          .map(s => Array.from(s).filter(
            op => Object.values(foundMapping).indexOf(op) === -1))
...
const program = inputOperationsRaw.slice(lastAfter * 4 + 1)
      .filter(l => l.length > 3)
